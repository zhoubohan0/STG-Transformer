...
import os
import imageio
from mineagent.common import network_utils
import argparse
from mineagent.official import build_pretrain_model
...
def main(args):
    ...
    # load clip model
    clip_config = network_utils.get_yaml_data(args.clip_config_path)
    ...
    # load task configs
    task_conf = network_utils.get_yaml_data(args.task_config_path)[args.task]
    #print(task_conf)
    init_items = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test-episode', type=int, default=5) # number of test episodes
    parser.add_argument('--seed', type=int, default=7) # random seed for both np, torch and env
    parser.add_argument('--save-snapshots', type=int, default=0) # save snapshots for each episode
    parser.add_argument('--save-gif', type=int, default=1) # save whole gifs for each episode
    parser.add_argument('--save-path', type=str, default='expert_traj') # save path for gifs and snapshots
    parser.add_argument('--clip-config-path', type=str, default='mineagent/official/config.yml') # config file for clip model
    parser.add_argument('--clip-model-path', type=str, default='mineagent/official/attn.pth') # model path for clip model
    parser.add_argument('--task-config-path', type=str, default='envs/hard_task_conf.yaml') # config file for task
    parser.add_argument('--skills-model-config-path', type=str, default='skills/load_skills.yaml') # config file for skills model
