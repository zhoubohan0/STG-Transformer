...
class SSTransformer(nn.Module):
    ...
    def update(self, data, timesteps):
        ...
        # 位置编码:局部(+全局)
        block_size = input_embedding.shape[1]
        position_encodings = self.pos_emb[:, :block_size, :]
        if timesteps is not None:
            timesteps = timesteps.clip(max=self.config.max_timestep - 1)
            position_encodings = position_encodings + self.global_pos_emb.repeat_interleave(batch_size,dim=0).gather(1,timesteps.repeat_interleave(self.config.n_embd,dim=-1))
        # 核心
    ...
    def trainss(self, src_root_list, threshold):
        ...
        # 训练
        save_dir = f'ablation-model/{task}_TDR/'
