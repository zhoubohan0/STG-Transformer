...
def muti_draw(files, length=None, xkey=None, ykey=None, labels=None, axislabel=('Transition', 'Episodic Return'),
    ...
    plt.grid()
    if save: plt.savefig(os.path.join(os.path.split(files[0])[0], ylabel + '_compare.jpg'), bbox_inches='tight',
                         dpi=300)
    plt.show()
...
def all_draw(inputseries, length=None, axislabel=('Transition', 'Smoothed Episodic Return'), labels=None, colors=None,
             save=None):
    plt.figure(figsize=(8, 6), dpi=300)
